...
class SubjectIndex {
    struct Node { Grade rec; Node* next; Node(const Grade& g):rec(g),next(nullptr){} };
    struct Bucket { QString subject; Node* head; Bucket():head(nullptr){} };
...
public:
    ...
    void rebuild(const SeqList<Grade>& list){
        clear(); for(size_t i=0;i<list.size();++i) add(list[i]);
    }
    void add(const Grade& g){
        Bucket* b=get_bucket(g.subject);
        Node* p=new Node(g); p->next=b->head; b->head=p;
    }
    bool remove_one(const Grade& g){
        for(size_t i=0;i<buckets.size();++i){
            ...
            while(*pp){
                ...
                { Node* del=*pp; *pp=del->next; delete del; return true; }
                pp=&((*pp)->next);
            }
        }
        return false;
...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent), datafile("test.csv") {
